...
export default function CardContacto({ id, userId, numero, objetivo, media, fechaInicio, fechaFin, estado, notesText }: CardContactoProps) {
  ...
  const formatMedia = (mediaRaw: string | null) => {
    if (!mediaRaw) return '-';
    switch (mediaRaw) {
      case 'email': return 'Mail';
      case 'number': return 'Número';
      default: return mediaRaw;
    }
  };
      ...
      <div className="space-y-3">
        <div className="flex gap-4">
          ...
          <div className="text-right">
            <span className="text-lux-muted text-xs uppercase tracking-widest block mb-1">Medio</span>
            <span className="text-white text-sm font-medium">{formatMedia(media)}</span>
          </div>
        </div>
